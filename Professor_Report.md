# IMU Calibration Sensitivity Analysis Report
...
- **도출된 결과**: 노이즈(Sigma)가 커질수록 구면 복원 방식의 오차(MSE) 곡선이 명확하게 더 가파르게 치솟았습니다. 반대로 바이어스(Bias) 증가 실험에서는 두 방법 모두 행렬 역산을 통해 안정적으로 일정하게 상쇄해내었습니다.
- **결과 원인(해석)**: 
...
- **도출된 결과**: 노이즈 분산에 비례하여 오차가 선형적으로 증가했을 뿐만 아니라, 초기 바이어스(Bias)가 커질 때도 최적화가 길을 잃고 오차가 크게 폭발하는 양상을 보였습니다.
- **결과 원인(해석)**: 
...
- **도출된 결과**: 노이즈 증가에 대해 가속도나 지자기에 비해 오차(MSE)가 아주 급격히 높아지는 노이즈의 취약성이 나타났습니다. 반면, 바이어스(Bias) 크기 증가 스윕에서는 오차가 늘지 않고 완전히 0의 바닥에 붙어서 철벽 방어를 하는 듯한 모습을 보였습니다.
